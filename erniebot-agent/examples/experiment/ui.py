...
parser.add_argument("--use_baizhong", type=bool, default=False, help="")
...
def generate_report(query, history=[]):
    ...
    #     )
    #     # abstract_search = BaizhongSearchTool(description="abstract", db=aurora_db_retriver_abstract)
    #     # retriever_search = BaizhongSearchTool(description="all paper", db=aurora_db_retriver)
    # else:
    paper_db = build_index(faiss_name=args.faiss_name_paper)
    abstract_db = build_index(faiss_name=args.faiss_name_abstract)
    abstract_search = FaissSearch(abstract_db)
    retriever_search = FaissSearch(paper_db)
